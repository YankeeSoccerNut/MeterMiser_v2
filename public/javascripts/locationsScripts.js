...
// To load up Google Maps in javascript versus src tag in HTML..
  var googMapURL = `https://maps.googleapis.com/maps/api/js?key=AIzaSyAD1SSIaneXQl7h7PyrkNOFy8KK7W7D5Zo&callback=initMap`;
...
// TODO: Add capability to use actual authenticated user credentials to limit locations their own
  ...
  function buildPage() {
    var getLocationsURL = '/api/Locations';
    ...
    $.get(getLocationsURL, function( locations, status ) {
      ...
      for (let i=0; i < locations.length; i++){
        ...
        var getCurrentReadingURL = '/api/Now?';

        getCurrentReadingURL += `thermostatId=${locations[i].thermostatId}`;
        console.log(getCurrentReadingURL);

        midSectionPromises.push($.get(getCurrentReadingURL, function (currentReading, status) {
      ...
      Promise.all(midSectionPromises).then(() => {
